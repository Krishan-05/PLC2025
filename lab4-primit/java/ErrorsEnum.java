import java.util.EnumSet;

public class ErrorsEnumSolution {
    
    public enum Error {
        FP_ROUNDING, FP_OVERFLOW, FP_UNDERFLOW, INT_OVERFLOW
    }

    public enum Result {
        A_BIT_DIFFERENT, INFINITY, ZERO, VERY_DIFFERENT
    }

    private static Result error2Result(Error e) {
        Result result = null;
        switch (e) {
            case FP_ROUNDING:
                result = Result.A_BIT_DIFFERENT;
                break;
            case FP_OVERFLOW:
                result = Result.INFINITY;
                break;
            case FP_UNDERFLOW:
                result = Result.ZERO;
                break;
            case INT_OVERFLOW:
                result = Result.VERY_DIFFERENT;
                break;
        }
        return result;
    }

    private static Error result2Error(Result r) {
        Error error = null;
        switch (r) {
            case A_BIT_DIFFERENT:
                error = Error.FP_ROUNDING;
                break;
            case INFINITY:
                error = Error.FP_OVERFLOW;
                break;
            case ZERO:
                error = Error.FP_UNDERFLOW;
                break;
            case VERY_DIFFERENT:
                error = Error.INT_OVERFLOW;
                break;
        }
        return error;
    }

    private static <T extends Enum<T>> T getEnumElement(String name, Class<T> enumType) {
        return Enum.valueOf(enumType, name);
    }

    public static void main(String[] args) {
        System.out.print("Known errors = ");
        for (Error e : EnumSet.allOf(Error.class)) {
            System.out.print(e + " ");
        }
        System.out.println();

        System.out.print("Known results = ");
        for (Result r : EnumSet.allOf(Result.class)) {
            System.out.print(r + " ");
        }
        System.out.println();

        Error e = getEnumElement("FP_OVERFLOW", Error.class);
        System.out.println(e + " results in: " + error2Result(e));

        Result r = getEnumElement("INFINITY", Result.class);
        System.out.println(r + " results from: " + result2Error(r));
    }
}
